...
export function runCanonicalCalculationTests(): { name: string; passed: boolean; error?: string }[] {
  ...
  // Test 18: ID-only / Blank Record (e.g., svgINN-ARC-WIR-SUR-01975) Handling
  test('ER-018: ID-only / Blank Records are excluded from KPI metrics, tracked in DataQualityLedger, and preserved in source', () => {
    const rows: SubmittalRow[] = [
      {
        id: '1',
      ...
      // ID-only / blank record: Valid docNo, but blank payload fields
      {
        id: '2',
    ...
    // Assert that active KPI metrics count ONLY the 2 valid records
    if (kpi.totalSubmittedSheets !== 2) throw new Error(`Expected 2 valid submitted sheets, got ${kpi.totalSubmittedSheets}`);
    if (kpi.totalUniqueDrawings !== 2) throw new Error(`Expected 2 valid unique items, got ${kpi.totalUniqueDrawings}`);
    if (kpi.approved !== 2) throw new Error(`Expected 2 approved items, got ${kpi.approved}`);

    // Assert that the ID-only record is tracked in DataQualityLedger
    if (kpi.dataQuality.blankRowWithIdCount !== 1) throw new Error(`Expected 1 blankRowWithId in DataQualityLedger, got ${kpi.dataQuality.blankRowWithIdCount}`);
    if (kpi.dataQuality.blankRowWithIdRecords[0]?.docNo !== 'svgINN-ARC-WIR-SUR-01975') {
      throw new Error(`Expected tracked ID svgINN-ARC-WIR-SUR-01975, got ${kpi.dataQuality.blankRowWithIdRecords[0]?.docNo}`);
    }

    // Assert that the sequence auditor identifies 01975 as ID-only record, NOT a missing sequence gap
    const audit = kpi.sequenceAudit;
    if (!audit) throw new Error('Sequence audit result missing');
    if (audit.totalBlankOrIdOnlyCount !== 1) throw new Error(`Expected totalBlankOrIdOnlyCount=1, got ${audit.totalBlankOrIdOnlyCount}`);
    if (audit.totalMissingCount !== 0) throw new Error(`Expected 0 missing gaps (01975 is present), got ${audit.totalMissingCount}`);
  });

  // Test 19: Distinguishing True Sequence Gaps vs Blank Records
  test('ER-019: True sequence gap (00009) is flagged as missing, while blank record (01975) is ID-Only exception', () => {
    const rows: SubmittalRow[] = [
      { id: '1', docNo: 'WIR-SUR-00001', rev: '00', sheetNo: '01', documentType: 'WIR-SUR', discipline: 'SUR', trade: 'Survey', workflowStage: 'Approved', status: 'A', submissionDate: '2026-01-01', dueDate: '', responseDate: '', logType: 'WIR-SUR', contractor: '', consultant: '', remarks: '', area: '', tradeSystem: '', isLatestRev: true, isRev0: true, delayDays: 0, overdue: false },
      // Note: WIR-SUR-00002 is absent (True Sequence Gap)
      { id: '2', docNo: 'WIR-SUR-00003', rev: '00', sheetNo: '01', documentType: 'WIR-SUR', discipline: 'SUR', trade: 'Survey', workflowStage: 'Approved', status: 'A', submissionDate: '2026-01-01', dueDate: '', responseDate: '', logType: 'WIR-SUR', contractor: '', consultant: '', remarks: '', area: '', tradeSystem: '', isLatestRev: true, isRev0: true, delayDays: 0, overdue: false },
      // WIR-SUR-00004 is physically present but ID-Only / Blank
      { id: '3', docNo: 'WIR-SUR-00004', rev: '', sheetNo: '', documentType: '', discipline: '', trade: '', workflowStage: '', status: '', submissionDate: '', dueDate: '', responseDate: '', logType: '', contractor: '', consultant: '', remarks: '', area: '', tradeSystem: '', isLatestRev: true, isRev0: false, delayDays: 0, overdue: false }
    ];
    ...
    const reg = audit.registerAudits['WIR-SUR'];
    if (!reg) throw new Error('WIR-SUR audit not found');

    // Expected sequence range: 1 to 4 -> total 4 expected
    if (reg.expectedPopulation !== 4) throw new Error(`Expected 4 expected population, got ${reg.expectedPopulation}`);
    // Actual valid rev0 items: 00001, 00003 -> 2
    if (reg.actualRev0Population !== 2) throw new Error(`Expected 2 actual rev0, got ${reg.actualRev0Population}`);
    // Missing sequence IDs: exactly WIR-SUR-00002 (1 missing)
    if (reg.missingCount !== 1) throw new Error(`Expected 1 missing gap (00002), got ${reg.missingCount}`);
    if (!reg.missingIds.includes('WIR-SUR-00002')) throw new Error(`Expected missingIds to include WIR-SUR-00002, got ${reg.missingIds.join(', ')}`);
    // Blank/ID-only items: exactly WIR-SUR-00004 (1 blank)
    if (reg.blankOrIdOnlyCount !== 1) throw new Error(`Expected 1 blankOrIdOnlyCount, got ${reg.blankOrIdOnlyCount}`);
