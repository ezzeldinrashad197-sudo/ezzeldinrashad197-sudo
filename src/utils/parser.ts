...
export const formatDate = (raw: unknown): string => {
  ...
  if (typeof raw === "string") {
    ...
    // 1. Explicit DD-MMM-YYYY or DD/MMM/YYYY or DD MMM YYYY
    // Example: 1-Aug-2026, 01-August-2026, 2-Aug-26
    ...
    // 2. Explicit MMM-DD-YYYY or MMM DD, YYYY
    // Example: Aug 1, 2026
    const mmmDYMatch = str.match(
    ...
    if (dmYMatch) {
      ...
      // Construction registers use DD/MM/YYYY as the standard convention.
      if (
        p2 >= 1 &&
        p2 <= 12 &&
        p1 >= 1 &&
...
export const parseExcelWorkbook = (
  wb: XLSX.WorkBook,
  fileName: string,
): SubmittalRow[] => {
  ...
  wb.SheetNames.forEach((sheetName) => {
    ...
    const getColIdx = (
      ...
      return headers.findIndex((h) => {
        ...
        return aliases.some(
          (alias) =>
            lower === alias || lower.includes(alias),
        );
      });
    };
    ...
    const colResponseDate = getColIdx(
      ...
      [
        ...
        "submit",
        "sent corrective",
        "sent date corrective",
      ],
    );

    const colCode = getColIdx([
      "code",
      "approval code",
      "code status",
      "status code",
    ...
    const colStatus = getColIdx([
      "status",
    ]);

    const colRemarks = getColIdx([
      "remarks",
      "comment",
    ]);

    const colArea = getColIdx([
      "area",
      "zone",
    ]);

    const colSystem = getColIdx([
      "system",
      "trade",
    ]);

    // NCR & SOR Specific Columns
    const colNcrRef = getColIdx([
      "ncr ref",
    ]);

    const colNcrLastRev = getColIdx([
      "last rev",
    ]);
    ...
    const colReviewTime = getColIdx([
      "review time",
    ]);

    const colTotalDuration = getColIdx([
      "total duration",
    ]);

    // Letters Specific Columns
    const colSubject = getColIdx([
      "subject",
    ]);

    const colDistributions = getColIdx([
      "distributions",
    ]);

    const colActionRequired = getColIdx([
      "action required",
    ]);

    const colHyperlink = getColIdx([
      "hyperlink",
    ]);
    ...
    rows.forEach((r, idx: number) => {
      ...
      /*
       * ROW-LEVEL DISCIPLINE SOURCE
       *
       * Primary source:
       *   actual Discipline/Trade column
       *
       * RFI fallback:
       *   historical RFI layout where discipline is stored at column index 3.
       */
      if (
        colDiscipline >= 0 &&
        r[colDiscipline]
       ...
       * evidence.
       *
       * UNCLASSIFIED is intentionally included here so that a generic
       * row can fall back to Composite Identity.
       */
      ...
      if (
         ...
         *
         * Reference-level discipline can still provide evidence when
         * neither row-level nor composite identity provides a valid
         * discipline.
         */
        const refString = (
          colNcrRef >= 0
      ...
      /*
       * Normalize the discipline AFTER the source precedence has been
       * resolved.
       *
       * Therefore an explicit RFI row discipline reaches normalization
       ...
       *
       * Explicit RFI row:
       *   contextDiscipline = normalized row discipline
       *
       * RFI without explicit row discipline:
       *   contextDiscipline = Composite Identity fallback
       *
       * Other registers:
      ...
      parsed.push({
        ...
        contractor:
          colContractor >= 0
            ? String(r[colContractor] || "").trim()
            : "",

        consultant:
          colConsultant >= 0
            ? String(r[colConsultant] || "").trim()
            : "",
        ...
        remarks:
          colRemarks >= 0
            ? String(r[colRemarks] || "").trim()
            : "",

        area:
          colArea >= 0
            ? String(r[colArea] || "").trim()
            : "",

        tradeSystem:
          colSystem >= 0
            ? String(r[colSystem] || "").trim()
            : "",

        // NCR & SOR Specific properties
        ncrRef:
          colNcrRef >= 0
            ? String(r[colNcrRef] || "").trim()
            : "",

        ncrLastRev:
          colNcrLastRev >= 0
            ? String(r[colNcrLastRev] || "").trim()
            : "",
        ...
        subject:
          colSubject >= 0
            ? String(r[colSubject] || "").trim()
            : "",

        sentDateCorrectiveAction:
          formatDate(
            colNcrSentDateCorrectiveAction >= 0
              ? r[colNcrSentDateCorrectiveAction]
              : "",
        ...
        responseTime:
          colResponseTime >= 0 &&
          !isNaN(Number(r[colResponseTime]))
            ? Number(r[colResponseTime])
            : undefined,

        reviewTime:
          colReviewTime >= 0 &&
          !isNaN(Number(r[colReviewTime]))
            ? Number(r[colReviewTime])
            : undefined,

        totalDuration:
          colTotalDuration >= 0 &&
          !isNaN(Number(r[colTotalDuration]))
    ...
    const baseDocType =
      mapped.workflowFamily === "LETTER"
        ? "LTR"
        : mapped.workflowFamily;
    ...
    let reportType =
      "Monthly / Cumulative KPI Dashboards (General Stats)";

    if (baseDocType.includes("RFI")) {
      reportType = "RFI Analytics Tab";
    } else if (
      baseDocType.includes("NCR")
    ) {
      reportType =
        "NCR Analytics (Excluded from General Stats)";
    } else if (
      baseDocType.includes("SOR")
    ) {
      reportType =
        "SOR Analytics (Excluded from General Stats)";
    } else if (
      baseDocType.includes("LTR") ||
      baseDocType.includes("LETTER")
    ) {
      reportType =
        "Correspondence Analytics (Excluded from General Stats)";
    } else if (
      baseDocType === "UNKNOWN"
    ) {
      reportType =
        "Workflow Mapping SSOT (Pending Action)";
    }
  ...
  console.group(
    `%c📊 [StructuSight Ingestion Trace Logs]`,
    "color: #D4AF37; font-weight: bold; font-size: 14px;",
  );

  traces.forEach((t) => {
    console.log(
      `%cWorksheet: %c${t.sheetName} %cin File: %c${t.fileName}`,
      "color: #94a3b8;",
    ...
    console.log(
      `%c  Detected Type : %c${t.detectedType} %c(Confidence: ${t.confidence}%)`,
      "color: #94a3b8;",
      "color: #38bdf8; font-weight: bold;",
      "color: #a7f3d0; font-style: italic;",
    );

    console.log(
      `%c  Assigned Type : %c${t.assignedType}`,
      "color: #94a3b8;",
      "color: #fbbf24; font-weight: bold;",
    );

    console.log(
      `%c  Normalized Type : %c${t.normalizedType}`,
      "color: #94a3b8;",
      "color: #a7f3d0; font-weight: bold;",
    );

    console.log(
      `%c  Calculation Type: %c${t.calculationType}`,
      "color: #94a3b8;",
      "color: #818cf8; font-weight: bold;",
    );

    console.log(
      `%c  Report Type     : %c${t.reportType}`,
      "color: #94a3b8;",
      "color: #34d399; font-weight: bold;",
    );

    console.groupCollapsed(
      `%c  Evidence Summary (${t.evidence.length} items)`,
      "color: #94a3b8; font-style: italic; cursor: pointer;",
    );
