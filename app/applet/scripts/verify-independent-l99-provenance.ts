...
const EXPECTED_GOLDEN_RAW_SHA256 = '8a5717d1e8c952dfaefbe2be47228a6f3a76dc8623adfb9b342fb2f87cc4f107';
const EXPECTED_GOLDEN_CANONICAL_SHA256 = '0df9f688e1bc989fce9d3434faeb66016be41dfed9be59ed9be2b07e8ef6e6df';
...
export function runIndependentProvenanceTest() {
  ...
  const reconstructedFindings: any[] = reconstructedRegisterData.findings || reconstructedRegisterData;
  ...
  for (let i = 0; i < 28; i++) {
    ...
    if (isMatch) {
      passCount++;
      console.log(`${orig.masterId} | ${orig.originalDomain} | ${orig.originalTitle} | ${tupleHash.substring(0, 16)}... | PASS`);
    } else {
      failCount++;
  ...
  if (fs.existsSync(GOLDEN_DATASET_PATH)) {
    ...
    const canonicalObj = dataset.map((item: any) => {
      const { checksum, ...rest } = item;
      return rest;
    });
    const sortedCanonical = canonicalObj.map((obj: any) => {
      const sorted: any = {};
      Object.keys(obj).sort().forEach(k => { sorted[k] = obj[k]; });
      return sorted;
    });
    const canonicalSha256 = crypto.createHash('sha256').update(JSON.stringify(sortedCanonical)).digest('hex');

    console.log(`Golden Dataset File: /src/test-datasets/GOLDEN_REGRESSION_BASELINE.json`);
    console.log(`Record Count:        ${dataset.length} records`);
    console.log(`Raw File SHA-256:    ${datasetSha256}`);
    console.log(`Canonical SHA-256:   ${canonicalSha256}`);
    console.log(`Embedded Checksum:   ${dataset[0]?.checksum || 'N/A'}`);
